...
def get_colorization_data(im, opt, model, device):
    '''
    ...
    with torch.no_grad():
        ...
        logits, reg = model(data['L'], torch.zeros_like(hints).to(device=device), torch.full_like(mask, -.5).to(device=device))
...
def forward(model, segmentation_model, opt, data):
    ...
    # Segmentation model needs (1, 212, 212) size input
    out_rgb = transforms.functional.resize(out_rgb, exp_config.image_size)
    out_rgb = normalize(out_rgb)
    out_rgb = out_rgb.mean(dim=1)
    out_rgb = torch.unsqueeze(out_rgb, 1)
    
    y_pred = segmentation_model(out_rgb)
    return out_rgb, y_pred


def compute_loss(opt, y, criterion, device):
    t = opt['target'].to(dtype=torch.long, device=device)
    loss = criterion(y, t)
    if not opt['targeted']:
        loss *= -1
    return loss
