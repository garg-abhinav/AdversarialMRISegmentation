    ...
    for i, (image) in enumerate(dataset_loader):
        ...
        for itr in range(attack_params['num_iter']):
            ...
            if attack_params['targeted']:
                if torch.allclose(torch.argmax(val, dim=1), target) and torch.mean(diff).item() > threshold and torch.mean((diff-prev_diff).abs()).item() < 1e-4:
                    print(f'[{itr+1}/{attack_params["num_iter"]}] Loss: {loss:.3f}, ' 
                      f'{torch.mean(diff).item():.5f}, {torch.mean((diff-prev_diff).abs()).item():.5f}')
                    break
            else:
                if not torch.allclose(torch.argmax(val, dim=1), target) and torch.mean(diff).item() > threshold and torch.mean((diff-prev_diff).abs()).item() < 1e-4:
                    print(f'[{itr+1}/{attack_params["num_iter"]}] Loss: {loss:.3f}, ' 
                      f'{torch.mean(diff).item():.5f}, {torch.mean((diff-prev_diff).abs()).item():.5f}')
                    break
            prev_diff = diff
